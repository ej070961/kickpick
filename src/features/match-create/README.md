...
새 경기를 생성하고 자동 포메이션 초안을 저장하는 feature입니다. `/matches/new` 화면의 2단계 wizard에서 경기 조건, 등록 선수 참가 여부, 경기 전용 게스트, 쿼터 보정 대상을 입력받아 `matches`, `match_players`, `match_guest_players`, `quarter_formations`, `formation_slots`를 생성합니다.
...
- `ui/MatchInfoFields.tsx`: 경기명, 날짜, 쿼터 수, 포메이션, 골키퍼 전 쿼터 고정 입력을 렌더링한다.
- `ui/PlayerSelectionModal.tsx`: 등록 선수 선택 모달과 등록 선수, 게스트, 총 참가 수 요약을 렌더링한다. 데스크톱에서는 테이블형 목록, 헤더 체크박스, 컬럼 정렬을 제공하고 모바일에서는 정렬 select와 목록 전체 참가 체크박스를 제공한다.
- `ui/GuestPlayerModal.tsx`: 해당 경기에서만 사용하는 게스트 관리 모달이다. 게스트를 추가, 수정, 삭제하고 완료 버튼으로 모달을 닫는다. 게스트 입력은 이름, 주 포지션, 부 포지션만 받으며 등번호는 저장하지 않는다. 우선순위는 UI 표시와 서버 저장 시 모두 참가자 최하위로 자동 배정한다.
...
- 골키퍼 전 쿼터 고정을 사용하려면 참가 선수에 `GK` 주 포지션 선수가 포함되어야 한다.
