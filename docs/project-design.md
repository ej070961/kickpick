...
- 경기 생성: 현재 2단계 wizard를 사용한다. 1단계는 경기/선수 설정, 2단계는 쿼터 보정 대상 선택이다.
- 포메이션 편집기: 쿼터 탭, 축구장, 후보 목록, 저장/내보내기 액션을 한 흐름으로 제공한다.
...
기본 템플릿은 코드의 `FORMATION_PRESETS`로 제공하고, 사용자는 `/formations`에서 팀별 템플릿을 추가/삭제할 수 있다.

- 템플릿은 항상 GK 포함 11개 슬롯이다.
- 생성 폼에서는 필드 슬롯 10개를 선택하고, GK는 자동 포함한다.
- 좌표는 `DEFAULT_SLOT_COORDS`를 기준으로 저장한다.
- 현재 기본 템플릿: `4-3-3`, `4-4-2`, `4-2-3-1`, `3-5-2`, `5-3-2`, `3-4-3`.
